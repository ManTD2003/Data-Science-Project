        ...
        while True:
                ...
                try:
                    ...
                    rating = len(
                        [
                            star
                            for star in stars
                            if "iconcmt-starbuy" in star.get_attribute("class")
                        ]
                    )
            ...
            try:
                ...
                next_button = driver.find_element(
                    By.XPATH,
                    f"//div[@class='pagcomment']/a[@title='trang {next_page_number}'][text()='›']",
                )
        ...
        try:
            ...
            try:
                product_name_element = driver.find_element(
                    By.CSS_SELECTOR, "div.product-name > h1"
                )
            ...
            try:
                product_price_element = driver.find_element(
                    By.CSS_SELECTOR, "div.bs_price > strong"
                )
                product_price = product_price_element.text
                data["price"] = product_price
            except:
                product_price_element = driver.find_element(
                    By.CSS_SELECTOR, "div.box-price > p.box-price-present"
                )
                product_price = product_price_element.text
                data["price"] = product_price
            ...
            # get product rating
            try:
                product_point_element = driver.find_element(
                    By.CSS_SELECTOR, "div.boxrate__top div.point > p"
                )
            ...
            # get reviews
            try:
                view_all_reviews_link = driver.find_element(
                    By.CSS_SELECTOR, "a.btn-view-all"
                )
...
def save_data(products_data, category):
    ...
    reviews_df = pd.DataFrame(
        reviews_list,
        columns=["reviewer_name", "review_content", "review_point", "product_id"],
    )
    if not os.path.exists(category):
        os.makedirs(category)
    products_df.to_csv(
        os.path.join(category, "products.csv"), index=False, encoding="utf-8"
    )
    reviews_df.to_csv(
        os.path.join(category, "reviews.csv"), index=False, encoding="utf-8"
    )
        ...
        try:
            see_more_button = wait.until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        "//strong[@class='see-more-btn' and contains(text(), 'Xem thêm')]",
                    )
                )
            )
